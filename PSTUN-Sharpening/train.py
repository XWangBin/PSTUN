...
parser = argparse.ArgumentParser(description="HSI Pansharpening Toolbox")
...
parser.add_argument("--outf", type=str, default='./exp/ourspan/', help='path log files')
parser.add_argument("--data_root", type=str, default='/home/sysadmin/PSTUN-Sharpening/datasets/')
    ...
    while iteration<total_iteration:
        ...
        for i, (images, labels,lrhsis) in enumerate(train_loader):
            ...
            if iteration %( per_epoch_iteration) == 0:
                ...
                print(" Iter[%06d], Epoch[%06d], learning rate : %.9f, Train loss: %.9f, ""Test RMSE: %.9f, Test PSNR: %.9f " % (iteration, iteration//per_epoch_iteration, lr, losses.avg, rmse_loss, psnr_loss))
                logger.info(" Iter[%06d], Epoch[%06d], learning rate : %.9f, Train Loss: %.9f, "
                      "Test RMSE: %.4f, Test PSNR: %.4f , Test SSIM: %.4f , Test ERGAS: %.4f " % (iteration, iteration//per_epoch_iteration, lr, losses.avg, rmse_loss, psnr_loss,ssim_loss,ergas_loss))
    return 0
